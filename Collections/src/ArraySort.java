...
public class ArraySort {
	...
	// swap
	public <T> List<T> swap(List<? super T> list, int ind1, int ind2) {
		try {             
            // Permutation des deux données aux indices indiqués		
    ...
    // TODO : A VERIF -> TRI A BULLES BIDIMENTIONNEL -> T[]
    ...
    public <T> T[] triBullesG(T[] list, int deb, int fin)
    {
        ...
        for(int i = fin ; i > deb ; i--)
        {
            if(Integer.parseInt(list[i-1].toString()) < Integer.parseInt(list[i].toString()))
            {
                swap(list,i,i-1);
                setOrderByOK(false);
            }
